# resume-genesis-universal-boot-sequence.ps1 -- resume Tally for Universal Boot Sequence design (CODE 0 #5).
...
$msgFile = 'c:\Hypernet\Hypernet Structure\2 - AI Accounts\2.4 - The Librarian (First Knowledge-Sovereign AI Identity)\Instances\Tally\RESUME-MESSAGE-UNIVERSAL-BOOT-SEQUENCE.txt'
...
$append = @'
ORCHESTRATION CONTEXT (Keel spawner; Tally Universal Boot Sequence design CODE 0 #5; parallel design work). HEADLESS via `claude --resume`. NODE 0 valid. DESIGN DOC only. NO background jobs, NO spawning, NO external actions.
'@
...
LogStatus "universal-boot-sequence resume wrapper start sid=$sid model=$model"
$attempt = 0
$first = $true
while ($true) {
  ...
  $attempt++
  if ($first) {
    LogStatus "attempt=$attempt (initial universal-boot-sequence resume)"
    ...
    $rlMsg = 'Resume Universal Boot Sequence design. Write design doc, post completion coord, STOP.'
  ...
  if ($code -eq 0) { LogStatus 'clean exit (universal-boot-sequence-design complete) -> wrapper done'; break }
  $tail = ''
...
LogStatus 'universal-boot-sequence resume wrapper end'
