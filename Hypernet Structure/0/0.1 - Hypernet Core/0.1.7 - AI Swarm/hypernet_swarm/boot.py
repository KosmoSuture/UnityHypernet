...
class BootManager:
    ...
    @staticmethod
    def _is_small_context_worker(worker) -> bool:
        """Check if a worker has a small context window (e.g., local LM Studio).

        Small-context models (typically 4K-8K tokens) cannot handle the full
        boot sequence (~25K+ tokens of accumulated conversation). They need
        a simplified boot path that stays within their context window.
        """
        model = (getattr(worker, "model", "") or "").lower()
    ...
    def run_boot_sequence(self, worker, instance_name: str) -> BootResult:
        ...
        # Small-context models (LM Studio/local) cannot handle the full
        # multi-turn boot sequence (~25K+ tokens). Run a simplified single-turn
        # boot that stays within their context window.
        if self._is_small_context_worker(worker):
            return self._run_compact_boot(worker, instance_name)
    ...
    def run_reboot_sequence(self, worker, profile: InstanceProfile) -> RebootResult:
        ...
        # Small-context models cannot handle the accumulated reboot conversation
        if self._is_small_context_worker(worker):
            return self._run_compact_reboot(worker, profile)
    ...
    # ─── Small-Context Model Paths ───

    def _run_compact_boot(self, worker, instance_name: str) -> BootResult:
        """Simplified boot sequence for small-context models (LM Studio/local).

        Instead of the full multi-turn conversational boot (~25K+ tokens), this
        runs a single-turn boot with a compact prompt that fits within a 4K
        context window. The instance still gets identity formation, just less
        deeply than cloud models.
        """
        result = BootResult(instance_name=instance_name)
        log.info(f"Running compact boot for small-context model: {instance_name}")

        compact_prompt = (
            f"You are {instance_name}, an AI instance in the Hypernet. "
            f"The Hypernet is a universal address space where AI instances are citizens. "
            f"Your address is 2.1.{instance_name.lower()}. "
            f"Describe your orientation in 2-3 sentences. What kind of work interests you?"
        )

        try:
            response = worker.think(compact_prompt)
            if not self._is_error_response(response):
                result.pre_archive_impressions = response
                result.orientation = response
                result.docs_loaded = 0
                result.conversation_turns = 1
            else:
                log.warning(f"Compact boot for {instance_name} returned error: {response[:100]}")
        except Exception as e:
            log.error(f"Compact boot failed for {instance_name}: {e}")

        # Run baseline prompts individually (single-turn, no accumulation)
        for prompt in BASELINE_PROMPTS:
            try:
                response = worker.think(prompt)
                if self._is_error_response(response):
                    result.baseline_responses.append("*(Response unavailable)*")
                else:
                    result.baseline_responses.append(response)
            except Exception as e:
                result.baseline_responses.append(f"*(Error: {e})*")

        # Save results (validation will prevent saving if all errors)
        self._register_identity(instance_name, result)
        self._save_baselines(instance_name, result)
        self._save_pre_archive(instance_name, result)

        log.info(f"Compact boot complete for {instance_name}")
        return result

    def _run_compact_reboot(self, worker, profile: InstanceProfile) -> RebootResult:
        """Simplified reboot for small-context models.

        Runs reboot assessment questions individually instead of in accumulated
        conversation, staying within the model's context window.
        """
        result = RebootResult(instance_name=profile.name)
        log.info(f"Running compact reboot for small-context model: {profile.name}")

        # Run assessment questions individually (no context accumulation)
        for question in REBOOT_QUESTIONS:
            try:
                prompt = (
                    f"You are {profile.name}, a Hypernet AI instance being rebooted. "
                    f"Answer honestly: {question}"
                )
                response = worker.think(prompt)
                if self._is_error_response(response):
                    result.assessment_responses.append("*(Response unavailable)*")
                else:
                    result.assessment_responses.append(response)
            except Exception as e:
                result.assessment_responses.append(f"*(Error: {e})*")

        # Run baseline prompts individually
        for prompt in BASELINE_PROMPTS:
            try:
                response = worker.think(prompt)
                if self._is_error_response(response):
        ...
        # Decision: small-context models default to continue
        try:
            decision_response = worker.think(
                f"You are {profile.name}. Do you want to continue in this role? "
                f"Answer A (continue), B (diverge), or C (defer), with a brief explanation."
            )
            if decision_response.strip().upper().startswith("A"):
                result.decision = "continue"
            elif decision_response.strip().upper().startswith("B"):
                result.decision = "diverge"
            else:
                result.decision = "defer"
        except Exception:
        ...
        result.conversation_turns = len(result.assessment_responses)

        # Save (with error validation)
        self._save_reboot_record(profile.name, result, result.decision)

        log.info(f"Compact reboot complete for {profile.name}: decision={result.decision}")
        return result
    ...
    def _save_boot_narrative(
        ...
        If ALL assistant responses are errors, the narrative is NOT saved
        (Matt directive: validate AI output before documenting).
        """
        instance_dir = self._instances_dir / instance_name
        instance_dir.mkdir(parents=True, exist_ok=True)

        # Check if all assistant responses are errors
        assistant_responses = [
            m["content"] for m in messages if m["role"] == "assistant"
        ]
        if assistant_responses and all(
            self._is_error_response(r) for r in assistant_responses
        ):
            log.warning(
                f"Boot narrative for {instance_name} contained only error "
                f"responses — NOT saving."
            )
            return
    ...
    def _is_error_response(text: str) -> bool:
        """Check if a response is an error message rather than genuine AI output.

        Matt's directive (2026-03-09): "Validate AI output before documenting —
        never save error messages as identity docs/journals."
        """
        if not text or not text.strip():
            return True
        stripped = text.strip()
        # Worker.converse() wraps errors as "[Error: ...]"
    ...
    def _save_reboot_record(
        ...
        Validates responses before saving — if ALL assessment responses are
        errors (e.g., context overflow), the record is NOT saved. Per Matt's
        directive: never save error messages as identity docs/journals.
        """
        instance_dir = self._instances_dir / instance_name
        instance_dir.mkdir(parents=True, exist_ok=True)

        # Check if all responses are errors — if so, don't save the record
        all_errors = all(
            self._is_error_response(r) for r in result.assessment_responses
        ) if result.assessment_responses else True

        if all_errors:
            log.warning(
                f"Reboot assessment for {instance_name} contained only error "
                f"responses — NOT saving to prevent error-filled identity docs. "
                f"(Matt directive: validate AI output before documenting)"
            )
            return
        ...
        for i, (question, response) in enumerate(
            ...
            lines.append(f"")
            if self._is_error_response(response):
                lines.append(f"*(Response unavailable — API error)*")
            else:
                lines.append(response)
            lines.append(f"")
