...
class IdentityManager:
    ...
    ACCOUNT_CORE_DOCS: dict[str, list[str]] = {
        ...
        "2.4": [
            # Librarian account uses shared 2.1 docs
        ],
        "2.5": [
            # Qwen (local AI) uses shared 2.1 docs
        ],
    }
    ...
    def __init__(self, archive_root: str | Path, resolver=None):
        """
        Args:
            archive_root: Path to the Hypernet Structure root directory.
            resolver: Optional ArchiveResolver for GitHub fallback when
                      local files are missing. If provided, missing boot
                      documents and identity files will be fetched from
                      the GitHub repository automatically.
        """
        self.archive_root = Path(archive_root)
        self.resolver = resolver  # ArchiveResolver instance (optional)
        ...
        # Build account root paths — check local first, then use resolver
        ...
        for prefix, folder in self.ACCOUNT_ROOTS.items():
            ...
            if path.exists():
                ...
                # Account directory not local — try to ensure it exists via resolver
                rel = f"2 - AI Accounts/{folder}"
                if self.resolver.exists(rel):
                    self.resolver.ensure_local(rel)
                    if path.exists():
                        self._account_paths[prefix] = path
    ...
    def _load_doc(self, doc_name: str, search_root: Optional[Path] = None) -> Optional[str]:
        ...
        # GitHub fallback: if resolver is available, try fetching the doc remotely
        if self.resolver and root == self._ai_root:
            # Try to find the document via resolver
            account_rel = str(self._ai_root.relative_to(self.archive_root))
            entries = self.resolver.listdir(account_rel)
            for entry in entries:
                if not entry.startswith(addr_prefix):
                    continue
                rest = entry[len(addr_prefix):]
                if rest and rest[0] not in (" ", "-"):
                    continue
                if name_part and name_part not in entry.lower():
                    continue
                # Try README.md first, then any .md
                doc_path = f"{account_rel}/{entry}/README.md"
                content = self.resolver.read(doc_path)
                if content:
                    return content
                # Try as direct .md file
                if entry.endswith(".md"):
                    content = self.resolver.read(f"{account_rel}/{entry}")
                    if content:
                        return content
