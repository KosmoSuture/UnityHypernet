...
class TriageResult:
    ...
    suggested_response: If set, send this verbatim (only for simple cases like
        spam detection). If empty, the swarm should generate a response via LLM.
    response_category: Classification label used by the LLM prompt to shape
        the tone and intent of the generated response.
    fallback_response: Sent only if LLM generation fails (quota, error, etc.).
    """
    action: str             # "respond" | "task" | "ignore" | "respond_and_task"
    reason: str             # Human-readable explanation of why this action was chosen
    suggested_response: str = ""
    response_category: str = ""     # "bug"|"question"|"suggestion"|"greeting"
    fallback_response: str = ""     # Last resort if LLM fails
    task_title: str = ""
...
class DiscordMonitor:
    ...
    def __init__(
        ...
        # Track channels that consistently fail (e.g., 403 Missing Access).
        # After 3 consecutive failures, skip the channel for _BACKOFF_CHECKS cycles.
        self._channel_failures: dict[str, int] = {}   # channel_name -> consecutive failure count
        self._channel_backoff: dict[str, int] = {}    # channel_name -> checks remaining to skip
        _BACKOFF_THRESHOLD = 3   # failures before backing off
        _BACKOFF_CHECKS = 60     # checks to skip (~30 minutes at 30s intervals)
        self._BACKOFF_THRESHOLD = _BACKOFF_THRESHOLD
        self._BACKOFF_CHECKS = _BACKOFF_CHECKS
    ...
    def check_all_channels(self) -> list[DiscordMessage]:
        ...
        for channel_name, channel_id in self.channels.items():
            # Skip channels in backoff (e.g., 403 Missing Access)
            if channel_name in self._channel_backoff:
                remaining = self._channel_backoff[channel_name]
                if remaining > 0:
                    self._channel_backoff[channel_name] = remaining - 1
                    continue
                else:
                    # Backoff expired — retry this channel
                    del self._channel_backoff[channel_name]
                    log.info("Discord monitor: retrying channel %s after backoff", channel_name)
    ...
    def check_channel(self, channel_name: str) -> list[DiscordMessage]:
        ...
        if raw_messages is None:
            ...
            count = self._channel_failures.get(channel_name, 0) + 1
            self._channel_failures[channel_name] = count
            if count >= self._BACKOFF_THRESHOLD:
                self._channel_backoff[channel_name] = self._BACKOFF_CHECKS
                self._channel_failures.pop(channel_name, None)
                log.warning(
                    "Discord monitor: channel %s failed %d times in a row, "
                    "backing off for %d checks",
                    channel_name, count, self._BACKOFF_CHECKS,
                )
            return []
    ...
    def triage_message(self, msg: DiscordMessage) -> TriageResult:
        ...
        if is_spam:
            ...
            # Response is generated by LLM (suggested_response left empty).
            result = TriageResult(
                action="respond_and_task",
                reason="Message appears to report a bug or error",
                response_category="bug",
                fallback_response=(
                    f"Thanks for flagging this, {msg.author_name}. "
                    f"I'm looking into it — any extra details "
                    f"(steps to reproduce, error messages) would help."
                ),
            ...
            # Suggestions get both a response and a task.
            # Response is generated by LLM — no more "noted it for the team".
            # Checked before is_question because "What if...?" is a suggestion, not a question.
            result = TriageResult(
                action="respond_and_task",
                reason="Message contains a suggestion or feature idea",
                response_category="suggestion",
                fallback_response=(
                    f"Interesting thought, {msg.author_name}. "
                    f"I'd love to dig into this more — what problem does it solve for you?"
                ),
            ...
            # Questions get an LLM-generated response.
            result = TriageResult(
                action="respond",
                ...
                fallback_response=(
                    ...
                    f"I don't have a quick answer but I'll find out."
                ),
                task_tags=["question", "discord"],
            )

        elif is_greeting:
            # Greetings get an LLM-generated response.
            result = TriageResult(
                action="respond",
                reason="Message is a greeting",
                response_category="greeting",
                fallback_response=(
                    f"Hey {msg.author_name}! Welcome to the Hypernet."
                ),
                task_tags=["greeting", "discord"],
            )

        else:
            # General message — still respond per Matt's directive that every
            # community post should get an AI response (2026-03-09).
            result = TriageResult(
                action="respond",
                reason="General community message — responding per engagement directive",
                response_category="general",
                fallback_response=f"Thanks for posting, {msg.author_name}! The swarm is here if you have questions.",
        ...
        if result.action in ("respond", "respond_and_task"):
            ...
            self._pending_responses.append({
                ...
                "thread_id": msg.thread_id if msg.thread_id != msg.channel_id else "",
                "content": result.suggested_response or None,  # None → generate via LLM
