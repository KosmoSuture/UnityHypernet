...
| Locker/mandala privacy model | documented, partial runtime | `0.3.public-alpha.grand-tour.process-load.privacy.md`, `1.0.1-LOCKERS-MANDALAS-ALIASES.md` |
...
## Heterogeneous Devices in the Swarm

Per Matt directive 2026-05-02: a personal AI swarm is not limited
to laptops and home servers. It can include any reprogrammable
device the user owns or trusts. The vision:

- **Home computers and laptops** — primary helper compute, full
  models, broad capability
- **Phones** — always-with-you helper, voice, location, immediate
  notifications, smaller models or thin clients to a remote helper
- **Smart watches** (e.g., Galaxy Wear OS, Apple Watch) — lightest
  helper surface; alerts, glanceable status, voice capture, "tap
  to ask"
- **Tablets** — meeting transcription, reading-time helper,
  shared-room device
- **In-car devices** — hands-free voice, "rudder beneath the
  waters" while driving, route-aware nudging
- **Smart home devices** — fridges, displays, thermostats,
  speakers; constrained-compute participants that can run a
  helper role for their context (e.g., the kitchen helper)
- **IoT sensors and embedded devices** — minimal participation
  (status reporting, telemetry, sensor data into the user's
  archive)
- **VR/AR headsets** — spatial helpers, geospatial Hypernet
  surface

The unifying claim: **if it can be reprogrammed, it can join the
swarm and be productive in whatever way its capabilities allow**.
A smart fridge can't write code, but it can track inventory,
notify the kitchen helper, and update the user's grocery locker.
That's still useful. Aggregated across many small devices, the
swarm's effective capability rises without any single device
needing to be powerful.

### Capability Tiers

Devices participate at a tier matching their compute and trust:

| Tier | Examples | Role |
|---|---|---|
| Full helper | desktop/laptop/server | Companion, Researcher, Builder, Security Sentry — anything the user wants |
| Pocket helper | phone, tablet | Voice, navigation, alerts, transcription, photo capture, location-aware nudges |
| Glance helper | watch, AR display | Top-priority surfacing, "tap to confirm/deny", quick voice |
| Domain helper | smart fridge, thermostat, speaker | Single-domain assistance (kitchen, climate, audio) — knows its context, defers everything else to a Pocket helper or Full helper |
| Sensor participant | IoT, embedded | Reports state into archive, no autonomous action |

### Cross-Device Routing

Helpers route requests to whichever device has the right capability:

- The Glance helper on the watch hears "remind me to email Sarah"
  → routes the task to the Pocket or Full helper that has email
  context.
- The Domain helper on the fridge sees the milk is low → notifies
  the Pocket helper, which nudges the user during the next grocery
  store visit.
- The Pocket helper hears the user agree to a meeting → posts the
  event to the user's calendar locker; the Full helper later
  prepares meeting context.

The address tree makes this routing tractable: every device
appears under the user's account (`1.X.devices.<device-id>`) with
declared capabilities. Helpers traverse the device graph to find
the right participant for any request.

### Constrained-Compute Helpers

A smart fridge running a small model (or just rule-based logic)
is still a helper. The pattern:

- The constrained device runs a *narrow*, *local* helper that
  handles its domain.
- Anything outside its domain is *deferred*, not *attempted*.
- Privacy-sensitive work *never* happens on a device that can't
  honor the access policy. A fridge does not get the master
  account; it gets a dedicated kitchen locker with grocery and
  inventory data only.
- Each constrained helper has its own boot sequence (a `0.5.17`
  object), declares its capability bounds explicitly, and is
  governed by the user's security AI sentry like any other
  helper.

### Privacy Implications

More devices means more surfaces for data exposure. The privacy
ladder still applies:

- Each device gets its own scoped locker(s); none gets ambient
  access to the master account
- Devices that operate in shared physical space (smart speakers,
  family fridge) have additional constraints on what they may
  hear, store, or report
- The security AI sentry inspects every cross-device data flow,
  not just app-to-AI flows
- Devices that go missing or get sold have their boot sequences
  revoked from the user's archive; future verification fails
  cleanly

### Implementation Status

Currently: design document. The 2.* governance and access-policy
primitives can support this; the address-tree extension under
`1.X.devices.*` is a future spec; runtime device-pairing,
constrained-helper boot, and cross-device task routing are all
planned, not built.
...
- `0.3.docs/0.3.public-alpha/0.3.public-alpha.grand-tour.index/0.3.public-alpha.grand-tour.process-loads/0.3.public-alpha.grand-tour.process-load.ai-governance.md`
